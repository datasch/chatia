...
import { resolveOutgoingJid } from "../services/WbotServices/IdentityResolverService";
...
import GenerateMessageSuggestionsService from "../services/MessageServices/GenerateMessageSuggestionsService";
...
export const transcribeAudioMessage = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    // Buscar mensagem pelo mediaUrl (nome do arquivo no banco)
    const message = await Message.findOne({
      where: { mediaUrl: fileName, companyId }
    });

    // Se já tem transcrição salva, retorna direto sem gastar API
    if (message && message.getDataValue("transcription")) {
      try {
        const saved = JSON.parse(message.getDataValue("transcription"));
        return res.json({
          transcribedText: saved.transcribedText || null,
          translatedText: saved.translatedText || null,
          detectedLanguage: saved.detectedLanguage || null
        });
      } catch (e) {
        // JSON inválido salvo, re-transcrever
      }
    }
...
- Respond ONLY with the JSON object`
    ...
    // Salvar transcrição no banco para não precisar transcrever novamente
...
export const aiSuggest = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    // Resolver UUID para ID numérico se necessário
    let numericTicketId = Number(ticketParam);
    if (isNaN(numericTicketId)) {
      ...
      numericTicketId = ticket.id;
    }

    const result = await GenerateMessageSuggestionsService({
    ...
    console.error("[AiSuggest] Erro:", err);
    return res.status(500).json({ error: "Erro ao gerar sugestões IA" });
  }
};
