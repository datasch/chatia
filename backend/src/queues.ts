...
// Configuração Redis para Bull - usar objeto em vez de URI
...
export const userMonitor = new BullQueue("UserMonitor", { redis: redisConfig });
export const scheduleMonitor = new BullQueue("ScheduleMonitor", { redis: redisConfig });
export const sendScheduledMessages = new BullQueue("SendSacheduledMessages", { redis: redisConfig });
export const campaignQueue = new BullQueue("CampaignQueue", { redis: redisConfig });
export const queueMonitor = new BullQueue("QueueMonitor", { redis: redisConfig });
export const contactReconcilerQueue = new BullQueue("ContactReconciler", { redis: redisConfig });

export const messageQueue = new BullQueue("MessageQueue", {
  redis: redisConfig,
  limiter: {
    ...
    const { count, rows: schedules } = await Schedule.findAndCountAll({
      where: {
        ...
        sendAt: {
          ...
          [Op.lte]: moment().utc().add("30", "seconds").format("YYYY-MM-DD HH:mm:ss")
        }
      },
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    if (schedule.valorIntervalo > 0 && (isNil(schedule.contadorEnvio) || schedule.contadorEnvio < schedule.enviarQuantasVezes)) {
      ...
      const timezone = await TimezoneService.getEffectiveTimezone(schedule.companyId);

      // Converter sendAt do UTC para timezone da empresa
      let novaData = moment.utc(schedule.sendAt).tz(timezone);

      console.log(unidadeIntervalo)
      // Adicionar intervalo usando moment-timezone
      if (unidadeIntervalo !== "minuts") {
        novaData.add(schedule.valorIntervalo, unidadeIntervalo);
      } else {
        novaData.add(Number(schedule.valorIntervalo), 'minutes');
        console.log(novaData.format())
      }
      ...
      const novaDataUTC = novaData.utc().format("YYYY-MM-DD HH:mm:ss");

      await scheduleRecord?.update({
        status: "PENDENTE",
        contadorEnvio: schedule.contadorEnvio + 1,
        sendAt: novaDataUTC // Mantendo o formato de hora
      })
    } else {
      await scheduleRecord?.update({
...
async function handleDispatchCampaign(job) {
  try {
    ...
    if (campaign.openTicket === "enabled") {
        ...
        if (campaign.confirmation && campaignShipping.confirmation === null) {
          ...
          if (campaign.mediaPath) {
            ...
            if (Object.keys(options).length) {
              // b) checando existência da prop + prefixo
            if ("mimetype" in options && typeof (options as any).mimetype === "string" && (options as any).mimetype.startsWith("audio/")) {
      ...
      if (campaign.whatsapp?.status !== "CONNECTED") {
        logger.error(`campaignQueue -> DispatchCampaign -> WhatsApp não conectado para campanha ${campaignId}`);
        return;
      }

      if (campaign.confirmation && campaignShipping.confirmation === null) {
        const sentMsg = await wbot.sendMessage(chatId, {
          text: `\u200c ${campaignShipping.confirmationMessage}`
        });
        if (sentMsg?.key?.id) {
          await campaignShipping.update({ confirmationRequestedAt: moment() });
        ...
        if (!campaign.mediaPath) {
          ...
          if (!sentMsg?.key?.id) {
            logger.error(`campaignQueue -> DispatchCampaign -> Falha ao enviar para ${campaignShipping.number}`);
            return;
          }
        }
            ...
            try {
              ...
              if (!isNil(t?.tag.nextLaneId) && t?.tag.nextLaneId > 0 && t?.tag.timeLane > 0) {
                ...
                const nextTag = await Tag.findOne({
                  where: {
                    id: t?.tag.nextLaneId,
                    companyId  // ✅ Validação multi-tenant
                  }
                });
                ...
                if (dataUltimaInteracaoChamado < dataLimite) {
                  ...
                  const io = getIO();
                  const ticketUpdated = await ShowTicketService(t.ticketId, companyId);
...
async function handleWhatsapp() {
  ...
  const defaultTimezone = await import("./services/TimezoneServices/TimezoneService").then(module =>
...
async function handleAiAutoSummary() {
  const job = new CronJob('*/1 * * * *', async () => {
    try {
      ...
      for (const company of companies) {
        try {
          const settings = await CompaniesSettings.findOne({
            ...
            attributes: ["aiAutoSummary", "aiAutoSummaryTime"]
          });

          if (!settings || settings.getDataValue("aiAutoSummary") !== "enabled") continue;

          const configuredTime = settings.getDataValue("aiAutoSummaryTime") || "23:00";
          const now = new Date();
          const currentTime = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;

          if (currentTime !== configuredTime) continue;

          logger.info(`[AiAutoSummary] Iniciando resumos automáticos para empresa ${company.id}`);

          const { default: CreateTicketAiSummaryService } = await import("./services/TicketAiSummaryService/CreateTicketAiSummaryService");
          ...
          for (const ticket of tickets) {
            try {
              await CreateTicketAiSummaryService({
                ticketId: ticket.id,
                companyId: company.id,
                userId: null,
              ...
              // Silenciar erros individuais (ex: sem mensagens novas)
            }
          }

          logger.info(`[AiAutoSummary] Empresa ${company.id}: ${tickets.length} tickets processados`);
        } catch (e: any) {
          logger.error(`[AiAutoSummary] Erro empresa ${company.id}: ${e.message}`);
        }
      }
    } catch (e: any) {
      logger.error(`[AiAutoSummary] Erro geral: ${e.message}`);
    }
  });
  job.start();
  logger.info("[AiAutoSummary] Cron job iniciado (verifica a cada 1 minuto)");
}
...
export async function startQueueProcess() {
  ...
  // Reconciliador de contatos: funde duplicatas e resolve pendências LID
  contactReconcilerQueue.process("Reconcile", async () => {
    try {
      const { runReconciler } = await import("./services/ContactServices/ContactReconcilerService");
      await runReconciler();
    } catch (e) {
      logger.error(`[Reconciler] Erro no job: ${e.message}`);
    }
  });

  contactReconcilerQueue.add(
    "Reconcile",
    {},
    {
      repeat: { cron: "*/3 * * * *", key: "contact-reconciler" },
      removeOnComplete: true
    }
  );
}