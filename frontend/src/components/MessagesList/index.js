...
import SaveFieldFromTextMenu from "../SaveFieldFromTextMenu";
...
const useStyles = makeStyles((theme) => ({
  ...
  messageLeft: {
    ...
    whiteSpace: "pre-wrap",
    backgroundColor: theme.mode === 'light' ? "#ffffff" : "rgba(20, 20, 34, 0.95)",
    ...
    boxShadow: theme.mode === 'light' ? "0 2px 8px rgba(0, 0, 0, 0.04)" : "0 4px 16px rgba(0, 0, 0, 0.35)",
  },

  quotedContainerLeft: {
    margin: "-3px -80px 6px -6px",
    overflow: "hidden",
    backgroundColor: theme.mode === 'light' ? "#f1f5f9" : "rgba(255, 255, 255, 0.05)",
    borderRadius: "10px",
    display: "flex",
    position: "relative",
  },
  ...
  messageRight: {
    ...
    background: theme.mode === 'light'
      ...
      : "linear-gradient(135deg, #082f49 0%, #0e3a52 100%)",
    color: theme.mode === 'light' ? "#0369a1" : "#f0fdfa",
    alignSelf: "flex-end",
    ...
    borderBottomRightRadius: 14,
    padding: "8px 12px 6px 12px",
    border: theme.mode === 'light' ? "1px solid #bae6fd" : "1px solid rgba(6, 182, 212, 0.3)",
    boxShadow: theme.mode === 'light' ? "0 2px 8px rgba(3, 105, 161, 0.08)" : "0 4px 16px rgba(6, 182, 212, 0.12)",
  },
...
const MessagesList = ({
  ...
  const [saveFieldOpen, setSaveFieldOpen] = useState(false);
  const selectionVirtualRef = useRef(null);
      ...
      const fetchMessages = async () => {
        ...
        try {
          ...
          if (currentTicketId.current === ticketId) {
            ...
            if (Object.keys(savedTranscriptions).length > 0) {
              setTranscriptions(prev => ({ ...prev, ...savedTranscriptions }));
            }
          } else {
...
const handleTextSelection = useCallback(() => {
  ...
  if (text.length >= 3 && contactId) {
    const range = selection.getRangeAt(0);
    const rect = range.getBoundingClientRect();
    selectionVirtualRef.current = {
      getBoundingClientRect: () => rect,
      clientWidth: rect.width,
      clientHeight: rect.height,
    };
    setSelectionText(text);
    setSelectionAnchorEl(selectionVirtualRef.current);
  } else {
                      ...
                      {xmlRegex.test(message.body) && (
                        <div>{formatXml(
                          message.translatedBody
                            ? (!message.fromMe
                              ? (showOriginal[message.id] ? message.body : message.translatedBody)
                              : (showOriginal[message.id] ? message.translatedBody : message.body))
                            : message.body
                        )}</div>

                      )}
                      {!xmlRegex.test(message.body) && (<MarkdownWrapper>{
                        message.translatedBody
                          ? (!message.fromMe
                            ? (showOriginal[message.id] ? message.body : message.translatedBody)
                            : (showOriginal[message.id] ? message.translatedBody : message.body))
...
return (
  <div className={classes.messagesListWrapper} onDragEnter={handleDrag}>
    ...
    {Boolean(selectionAnchorEl) && selectionText && (
      <Popper
        open={true}
        ...
        style={{ zIndex: 9999 }}
      >
        <ClickAwayListener onClickAway={() => { setSelectionAnchorEl(null); setSelectionText(""); }}>
          <Paper
            className={classes.saveFieldFab}
            onClick={() => { setSaveFieldOpen(true); setSelectionAnchorEl(null); }}
          >
    ...
    <SaveFieldFromTextMenu
      open={saveFieldOpen}
      onClose={() => { setSaveFieldOpen(false); setSelectionText(""); }}
