                    ...
                    secondary={
                        ...
                        <MarkdownWrapper>{i18n.t("messagesResponsive.ads.adClick")}</MarkdownWrapper> //Clique de Anúncio adicionado
                                ...
                                <span className={classes.secondaryContentSecond} >
                                    {ticket?.whatsapp ? <Badge className={classes.connectionTag} style={{ backgroundColor: ticket.channel === "whatsapp" ? "#05ab92" : ticket.channel === "facebook" ? "#4267B2" : "#E1306C" }}>{ticket.whatsapp?.name.toUpperCase()}</Badge> : <br></br>}
                                    ...
                                    {ticket?.user && (<Badge style={{ backgroundColor: "#055bab" }} className={classes.connectionTag}>{ticket.user?.name.toUpperCase()}</Badge>)}
                                    {ticket.tags && ticket.tags.length > 0 && ticket.tags.map((tag) => (
                                        <Badge style={{ backgroundColor: tag.color || "#999" }} className={classes.connectionTag} key={`ticket-tag-${ticket.id}-${tag.id}`}>{tag.name.toUpperCase()}</Badge>
                                    ))}
                                    {ticket.contact?.tags && ticket.contact.tags.length > 0 && ticket.contact.tags.map((tag) => (
                                        <Badge style={{ backgroundColor: tag.color || "#999" }} className={classes.connectionTag} key={`contact-tag-${ticket.id}-${tag.id}`}>{tag.name.toUpperCase()}</Badge>
                                    ))}
                                </span>
                            </Typography>
